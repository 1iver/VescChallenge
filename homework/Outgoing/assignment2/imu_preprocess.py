...
def func_calculation(data, func_size, func):
    ...
    return func_data
# %%
file = "MyStepCounter/acc_data/walk/IMU-2023-04-27-09_26_10_walk_120step.txt"
file = "MyStepCounter/acc_data/trot/IMU-2023-04-27-09_28_19_trot_189step.txt"
# file = "MyStepCounter/acc_data/sit/IMU-2023-04-27-10_28_25_sit.txt"
# file = "AccData/sit/IMU-2023-05-06-09_58_36_squats.txt"
# file = "AccData/wave/IMU-2023-05-06-10_22_53_wave.txt"
data_len = 100000
...
acc_data[:, 0] = data[0:cnt:3]
acc_data[:, 1] = data[1:cnt:3]
acc_data[:, 2] = data[2:cnt:3]
acc_data[:, 3] = (acc_data[:, 0] ** 2 + acc_data[:, 1] ** 2 + acc_data[:, 2] ** 2) ** 0.5  # magnitude

m1 = 11  # moving window, func_calculation for keep size
...
# %%
plt.figure(11)
plt.plot(acc_xyz_mean[:, 2], 'g')
plt.plot(acc_xyz_mean[:, 1], 'r')
plt.plot(acc_xyz_mean[:,0], 'b')
plt.plot(acc_xyz_mean[:,3], 'y')
plt.grid(True)
plt.show()
# %%
plt.psd(acc_xyz_mean[:, 2],Fs=25)
plt.psd(acc_xyz_mean[:, 1],Fs=25)
plt.psd(acc_xyz_mean[:, 0],Fs=25)
...
class action_processor:
    ...
    def find_possible_peak_valley(self, a):
        ...
        for i in range(1, a_len - 1):
            if a[i] >= a[i - 1] and a[i] > a[i + 1]:   # 增加了close point
...
# %%
axis_label = ['x', 'y', 'z']
delay_point = m1 // 2 + m2 // 2  #延迟采集时长
for i in range(fs+delay_point,acc_len,fs):
    xyz_sec = acc_xyz_mean[(i-fs):i,0:3]   # 模拟每次缓存1s的数据，构建数据窗口，当获得足够的数据时，分析步数
    ...
    if win_cnt>=win_len:
        for j in range(3):
            ...
            if tmp_diff > STEP_ACC_DIFF_THRESHOLD:  # 相关参数一，排除sit
                ...
                plt.grid(True)
                plt.title(
                    str(i) + '    possible peaks valleys:    ' + axis_label[j] + ':    ' + str(axis_process.p_cnt))

                axis_process = axis_process.remove_false_peak_valley(axis_win)   # 数量多时的情况, remove false peaks性能一般
                ...
                if axis_process.v_cnt > 0:
                    plt.plot(axis_process.v_loc, axis_win[axis_process.v_loc], 'bo')
                plt.grid(True)
                plt.title(str(i) + '    remove false peaks valleys:    ' + axis_label[j] + ':    ' + str(
                    axis_process.p_cnt))

                axis_process = axis_process.merge_close_peaks_valleys(axis_win)#缺少direction 参数
                ...
                if axis_process.v_cnt > 0:
                    plt.plot(axis_process.v_loc, axis_win[axis_process.v_loc], 'bo')
                plt.grid(True)
                plt.title(str(i) + '    merge close peaks valleys:    ' + axis_label[j] + ':    ' + str(
                    axis_process.p_cnt))
                ...
                plt.grid(True)
                plt.title(
                    str(i) + '    remove asymmetric peaks: ' + axis_label[j] + ':    ' + str(axis_process.p_cnt))
                # plt.tight_layout()
                plt.show()

                action_num[j] = axis_process.p_cnt + axis_process.v_cnt
        xyz_win = np.zeros((win_len, 3))
        win_cnt = int(0)
        step_total += int(np.median(action_num))
        print("time = %3d(s)   idx = %3d   x = %3d   y = %3d   z = %3d   step = %d   total = %d" % (
            i // fs, i, action_num[0], action_num[1], action_num[2], int(np.median(action_num)), step_total))
