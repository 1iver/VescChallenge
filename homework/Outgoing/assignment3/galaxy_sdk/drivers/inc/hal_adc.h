...
typedef struct {
    ...
    bool dma_cyclic; /**< DMA cyclic mode enable or disable */
} AdcChannelsCfg;
...
typedef struct AdcOperations {
    ...
    int (*window_compare)(const AdcDevice *device,
                          const AdcSamplingConfig *samp_cfg,
...
/**
 * @brief Start window compare, only detection signal，no need to get data
 * @param device an instance of ADC device
 * @param samp_cfg the configuration of sampling
 ...
 * @retval int VSD_SUCCESS for succeed, others for failure
 */
int hal_adc_window_compare(const AdcDevice *device,
                           const AdcSamplingConfig *samp_cfg,
                           const AdcChannelsCfg *channels_cfg);
