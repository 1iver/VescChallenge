...
typedef struct DmacOperation {
    int (*get_resource)(const DmacDevice *device, DmacDevType src,
                        DmacDevType dst);
    int (*free_resource)(const DmacDevice *device, DmacXferCfg *xfer_cfg);
    ...
    int (*stop)(const DmacDevice *device, uint8_t chn_id);
 ...
 * @param[in] mux_id peripheral mux id, designed by soc
...
/**
 * @brief Free a assigned dmac channel
 * @param[in] device the Dmac device
 * @param[in] xfer_cfg channel configuration
 * @return Return VSD_SUCCESS for succeed, others for failure
 */
int hal_dmac_free_resource(const DmacDevice *device, DmacXferCfg *xfer_cfg);
 ...
 * @param[in] device the Dmac device
 * @param[in] chn_id  - channel id which will be stop
 * @return Return VSD_SUCCESS for succeed, others for failure
 */
int hal_dmac_stop(const DmacDevice *device, uint8_t chn_id);
